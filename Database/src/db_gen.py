...
class SQLCreate:
    artist_tb = """ CREATE TABLE IF NOT EXISTS artist (
        id INTEGER NOT NULL PRIMARY KEY,
        name TEXT NOT NULL
    );
    """
    album_tb = """ CREATE TABLE IF NOT EXISTS album (
        id INTEGER NOT NULL PRIMARY KEY,
        name TEXT NOT NULL,
        release_date INT,
        album_type INT
    ...
    """
    credit_tb = """ CREATE TABLE IF NOT EXISTS credit (
    ...
    """
    queue_track_tb = """ CREATE TABLE IF NOT EXISTS queue_track (
    ...
    playlist_tb = """ CREATE TABLE IF NOT EXISTS playlist (
        user_id INT NOT NULL REFERENCES user(id) ON DELETE CASCADE,
        name TEXT NOT NULL,
        description TEXT,
    ...
    """
    playlist_content_tb = """ CREATE TABLE IF NOT EXISTS playlist_content (
...
class SQLInsert:
    ...
    def new_user(conn, user):
        ...
        New user
        :conn: connection
        :user: (online,display_name,repeat_state,is_playing,cur_queue_idx,cur_progress)
        """
        sql = """ INSERT INTO user(online,display_name,repeat_state,is_playing,cur_queue_idx,cur_progress)
            VALUES(?,?,?,?,?,?);
        """
    ...
    def new_queue_track(conn, queue_track):
        """
        New queue_track
        :conn: connection
        :queue_track: (user_id,album_id,track_number,queue_index)
        """
        sql = """ INSERT INTO queue_track(user_id,album_id,track_number,queue_index)
            VALUES(?,?,?,?);
        """
        return SQLInsert.__ins__(conn, sql, queue_track)
...
def make_db_tables(conn):
    ...
    create_table(conn, SQLCreate.queue_track_tb)
...
def add_test_data(conn):
    ...
    # example test
    begin_trans(conn)
    SQLInsert.new_artist(conn, ("shit_artist",))  # auto id okie <3
    ...
    SQLInsert.new_user(conn, (0, "shit_user", 0, 0, 0, 0))
    ...
    SQLInsert.new_queue_track(conn, (1, 1, 0, 0))
    SQLInsert.new_queue_track(conn, (1, 1, 1, 2))
...
if __name__ == "__main__":
    conn = create_connection(r".\..\db\sqlitedb.db")
    if conn is not None:
        print("DB Connected")
